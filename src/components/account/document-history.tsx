...
/** Everything the company has ever sent for a document, newest first. */
    ...
    <div className="max-w-full overflow-x-auto">
      <table className="w-full min-w-[520px] text-sm">
        ...
        <tbody className="divide-y divide-border">
          {rows.map((document) => {
            const display =
              document.status === 'approved' ? documentDisplayStatus(document.valid_until) : null;
            return (
              <tr key={document.id}>
                ...
                <td className="py-2.5">
                  {display ? (
                    <StatusBadge tone={DISPLAY_STATUS[display].tone}>
