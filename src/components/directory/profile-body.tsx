        ...
        {documents.length > 0 ? (
          <ul className="flex flex-col gap-3">
            {documents.map((document) => (
              ...
              >
                {/* A basis, not `flex-1` (basis 0): in a wrapping row a zero
                    basis never wraps, and on a phone the document's name was
                    squeezed to one word per line beside its badge. */}
                <span className="min-w-0 flex-[1_1_12rem] text-body">{document.label}</span>
                {document.validMonth ? (
                  <span className="text-small text-muted">
