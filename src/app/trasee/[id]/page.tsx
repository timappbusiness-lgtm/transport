...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[64rem] px-[clamp(16px,4vw,56px)] py-10 sm:py-14">
      ...
      {departed || full ? (
        <p className="mt-6 rounded-input border border-warning/40 bg-warning/8 px-4 py-3 text-body">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)]">
        <div className="flex flex-col gap-6">
          <section className="rounded-card border border-border bg-surface p-5">
            ...
            <ol className="mt-4 flex flex-col gap-3">
              {cities.map((city, index) => (
                <li key={`${city}-${index}`} className="flex items-center gap-3 text-body">
                  <span
                    aria-hidden="true"
          ...
          {departure.platform_slots_total !== null ? (
            <section className="rounded-card border border-border bg-surface p-5">
              ...
              <p className="mt-1 text-body text-muted">{seatsSentence(departure)}</p>
          ...
          <section className="rounded-card border border-border bg-surface p-5">
            ...
            <dl className="mt-3 flex flex-col gap-3">
              {departure.service_types.map((service) => (
                <div key={service}>
                  <dt className="text-body font-medium">{SERVICE_TYPE_LABELS[service]}</dt>
                  <dd className="text-body text-muted">{SERVICE_TYPE_NOTES[service]}</dd>
                </div>
              ))}
            </dl>

            <h2 className="mt-6 text-body font-medium">{c.accepts}</h2>
        ...
        <aside className="flex flex-col gap-6 lg:sticky lg:top-24 lg:self-start">
          ...
          <section className="rounded-card border border-border bg-surface p-5">
            ...
            {carrier ? (
              <div className="mt-3">
                <p className="text-body font-medium">{carrier.name}</p>
      ...
      {departed || full ? (
        <section className="mt-12">
          <h2 className="text-h3">{c.similar}</h2>
          {similar.length > 0 ? (
            ...
            <p className="mt-2 text-body text-muted">{c.noSimilar}</p>
          )}
        </section>
      ) : null}
    </div>
