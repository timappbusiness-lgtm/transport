 ...
 * `src/config/brand.ts`, the mark in `src/config/brand-mark.ts`, the
 * operator in `src/config/company.ts`, or a colour token in
 * `src/app/globals.css`, and commit what it writes:
 *
 ...
 *   supabase/functions/_shared/brand.ts   the name, the mark and the operator line for the edge functions
...
import { operatorShortLine } from '../../src/config/company.ts';
...
export function edgeMirror(): string {
  return `// Generated by \`pnpm brand\` from src/config/brand.ts, brand-mark.ts and company.ts.
...
export const OPERATOR_LINE = ${JSON.stringify(operatorShortLine())};
...
export function record(root: string): string {
  const inputs = createHash('sha256')
    .update(JSON.stringify({ name: BRAND_NAME, tagline: BRAND_TAGLINE_RO, mark: MARK, operator: operatorShortLine(), tokens: readTokens(root) }))
